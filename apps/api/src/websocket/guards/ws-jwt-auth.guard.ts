...
export class WsJwtAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    try {
      ...
      if (!user) {
        this.logger.warn(
          `WebSocket connection attempt with invalid user ID: ${payload.sub}`,
        );
      ...
      if (!user.isActive) {
        this.logger.warn(
          `WebSocket connection attempt from inactive user: ${user.id}`,
        );
      ...
      if (!user.isVerified) {
        this.logger.warn(
          `WebSocket connection attempt from unverified user: ${user.id}`,
        );
      ...
      this.logger.log(
        `WebSocket authentication successful for user: ${user.username} (${user.id})`,
      );
      return true;
    } catch (error) {
      const errorMessage =
